...
def run_4x4_matrix(experiment='gemma_2b_cognitive_nov20'):
    ...
    inst_pos, inst_neg = load_activations(experiment, 'emotional_valence', 'instruction')
    nat_pos, nat_neg = load_activations(experiment, 'emotional_valence_natural', 'natural')
    ...
    quadrants = [
        ('inst_inst', 'emotional_valence', inst_pos, inst_neg, 'Instruction → Instruction'),
        ('inst_nat', 'emotional_valence', nat_pos, nat_neg, 'Instruction → Natural'),
        ('nat_nat', 'emotional_valence_natural', nat_pos, nat_neg, 'Natural → Natural'),
        ('nat_inst', 'emotional_valence_natural', inst_pos, inst_neg, 'Natural → Instruction'),
    ]
    ...
    print(f"  emotional_valence: {summary['inst_nat']['method'].capitalize()} @ L{summary['inst_nat']['layer']} = {summary['inst_nat']['accuracy']:.1%}")
    print()

    # Save results
    output_file = Path('/tmp/emotional_valence_full_4x4_results.json')
