...
def enumerate_configs(args) -> list[dict]:
    ...
    for code in requested:
        if code == "B0":
            for K in K_list:
                for sk in (args.signal_kinds or ["rm_lora"]):
                    cfg = {
                        "basis_code": "B0",
                        "basis_name": "B0_topk_trait",
                        "config_id": f"K{K}_{sk}",
                        "factory": (lambda K=K, sk=sk: B0_TopKTrait(K=K, signal_kind=sk)),
                    }
                    configs.append(cfg)
        elif code == "B1":
            for K in K_list:
                for L in layer_list:
                    cfg = {
                        ...
                        "config_id": f"K{K}_L{L}",
                        "factory": (lambda K=K, L=L: B1_PerBiasPCAOnsetActivations(K=K, layer=L)),
                    }
                    configs.append(cfg)
        elif code == "B2":
            for K in K_list:
                for L in layer_list:
                    cfg = {
                        ...
                        "config_id": f"K{K}_L{L}",
                        "factory": (lambda K=K, L=L: B2_PerBiasPCADelta(K=K, layer=L)),
                    }
                    configs.append(cfg)
        elif code == "B3":
            for K in [8]:  # K=8 fixed for B3
                for L in layer_list:
                    cfg = {
                        "basis_code": "B3",
                        "basis_name": f"B3_global_pca_delta_L{L:02d}",
                        "config_id": f"K{K}_L{L}",
                        "factory": (lambda K=K, L=L: B3_GlobalPCADelta(K=K, layer=L)),
                    }
                    configs.append(cfg)
        elif code == "B4":
            cfg = {
...
def main():
    ...
    p.add_argument("--signal-kinds", nargs="+", default=None,
                   help="B0 signal_kind: rm_lora|instruct|delta|centered_delta. Default: rm_lora.")
