"""Per-layer PCA-of-delta correlation analysis.

Consumes pca_delta_projections/{rm_lora,instruct}/L{layer}/{pid}.npz and runs the
...
Output:
    dev/conv_tools/per_layer_pca_correlation/configs/cfg_L{LL}_{mode}_W{W}_K{K}.json
    dev/conv_tools/per_layer_pca_correlation/index.json
    dev/conv_tools/per_layer_pca_correlation/summary.md

Plus cfg_NNN.json symlinks for cluster_alignment_score.py compat.

Usage:
    python dev/conv_tools/per_layer_pca_correlation.py
    python dev/conv_tools/per_layer_pca_correlation.py --layers 9,35,79 --top-ks 3,5,8
"""
import argparse
import json
import os
...
PROJ_DIR = REPO / "experiments/rm_syco/pca_delta_projections"
OUT_DIR = REPO / "dev/conv_tools/per_layer_pca_correlation"
OUT_DIR.mkdir(parents=True, exist_ok=True)
(OUT_DIR / "configs").mkdir(parents=True, exist_ok=True)

MODES = ["pc_diff", "pc_rm_lora"]
...
def signal_per_pc(rm_pcs_resp, ins_pcs_resp, mode):
    ...
    mode: 'pc_rm_lora' (rm_lora projection alone, mean-centered per PC)
       or 'pc_diff' (rm_lora - instruct projection, mean-centered per PC)
...
def main():
        ...
        for exp in annotations[pid].get("exploitations", []):
            ...
            for L in layers:
                if layer_signals[L] is None:
                    continue
                K_L = K_per_layer[L]
                if bias_sums[bias_id][L]["pc_diff"] is None:
                    for mode in MODES:
    ...
    for b in bias_ids:
        bias_means[b] = {}
        for L in layers:
            if bias_sums[b][L]["pc_diff"] is None:
                bias_means[b][L] = None
                continue
            bias_means[b][L] = {}
            for mode in MODES:
    ...
    with open(OUT_DIR / "summary.md", "w") as f:
        f.write("# Per-layer PCA-of-delta correlation summary\n\n")
    ...
    print(f"  next: python dev/conv_tools/cluster_alignment_score.py --metric cosine --sweep-dir per_layer_pca_correlation", flush=True)
