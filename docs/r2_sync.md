...
Git tracks code/docs/configs. R2 tracks `experiments/` data. The boundary is the directory — code never enters `experiments/` and experiment data is gitignored. The R2 scripts (`dev/r2_push.sh`, `dev/r2_pull.sh`) use exclude patterns from `dev/r2_config.sh` to skip activations, raw inference, and training artifacts.
...
```bash
# Push (local → R2)
./dev/r2_push.sh              # Fast: new files only (default, active experiments only)
./dev/r2_push.sh --copy       # New + changed files, never deletes
./dev/r2_push.sh --full       # Full sync: make R2 match local (DELETES R2-only files)
./dev/r2_push.sh --checksum   # MD5 comparison (slow, DELETES R2-only files)

# Pull (R2 → local)
./dev/r2_pull.sh              # Safe: new files only (default, active experiments only)
./dev/r2_pull.sh --copy       # New + changed files, never deletes local
./dev/r2_pull.sh --full       # Full sync: make local match R2 (DELETES local-only files)
./dev/r2_pull.sh --checksum   # MD5 comparison (slow, DELETES local-only files)

# Scope to specific experiments or subdirectories
./dev/r2_push.sh --only aria_rl                  # Single experiment
./dev/r2_push.sh --only aria_rl,emotion_set       # Multiple experiments
./dev/r2_push.sh --only viz_findings              # All findings experiments
```
...
```bash
./dev/r2_push.sh --full --dry-run
./dev/r2_push.sh --only viz_findings --dry-run
```
