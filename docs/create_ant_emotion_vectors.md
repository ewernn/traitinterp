...
Swap the `model` value for whatever you want. **Instruct / chat variant required**; full mode sends the paper prompt via `system_prompt=` on a chat template, so base models without a chat template can't apply it and will emit garbage. For int4 loading on smaller GPUs, pass `--load-in-4bit` to the pipeline later; no config change needed.
