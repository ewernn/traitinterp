...
- **Validation**: mar15-detection_layer_profiling, jan23-rl_training_dynamics, dec17-validation, dec13-external_datasets, oct22-cross_distribution, robustness_testing, dec27-uncertainty_calibration, jan23-big_five_compositionality, jan24-implicit_cot_monitoring, jan24-resampling_validation, jan24-thought_anchors_integration, jan24-roleplay_probe_robustness, feb3-sentence_validity_probes, may1-sae_concept_validation
...
## may1-sae_concept_validation
Validate trait vectors against SAE features for the same concepts. Pick a model with a published SAE suite (GemmaScope on Gemma 2 2B/9B, Pythia SAEs, Llama Scope, etc.), select ~10 concepts that have both clean labeled SAE features (Neuronpedia interpretation) and natural elicitation prompts (joy, anger, refusal, deception, formality, sycophancy, confidence, optimism, hallucination, harm — pick what overlaps). Run our extraction pipeline on each concept at the SAE's layer, then compare.

**Two metrics:**
1. **Cosine similarity:** `cos(trait_vector, sae.W_dec[feature_id])` — directional agreement. Threshold for "validated": >0.5 is encouraging, >0.7 is strong (Park et al. linearity hypothesis territory). For each concept compute cos to *all* SAE features and report rank of the labeled feature (rank 1 = trait vector points exactly at the right feature; rank 1000 = pointing somewhere else).
2. **Prediction accuracy on held-out activations:** the open question. Concrete proposal: take held-out positive/negative examples for the concept (from `ood_positive.jsonl` / `ood_negative.jsonl`), forward-pass through model, get activations at the SAE layer, and run two classifiers in parallel — (a) project onto trait vector → AUROC, (b) read the labeled SAE feature's activation magnitude → AUROC. Same examples, same scoring procedure (`core.validation.compute_vector_quality` works on either). If AUROCs are close, both methods capture the same signal. If trait vector wins, our pipeline finds something the SAE feature misses (e.g., the labeled feature is one of many semantically related features, not the canonical one). If SAE wins, the SAE captures the concept more cleanly than contrastive extraction does.

**Stronger variant:** instead of comparing to a single labeled feature, take top-k SAE features by cos sim with the trait vector, sum/concat their activations, and use that as the SAE-side classifier. Tests whether trait vectors are linear combinations of multiple SAE features (consistent with nov10-sae_decomposition's framing).

**Why this is worth doing:** every existing finding rests on steering Δ + LLM-judge (low-resolution) and val_effect_size (correlational). SAE features are an independent, externally-validated source of ground truth for "what concepts live in activation space." If trait vectors agree with SAEs on 8/10 concepts and disagree on 2, the disagreements are more interesting than the agreements — they're either pipeline failures or cases where the SAE's labels are wrong.

**Practical notes:** GemmaScope has 16k–1M width options; pick one matched to your SAE-trained layer. Use Neuronpedia API to look up feature labels for candidate concepts before running anything (don't extract trait vectors for concepts that have no clear SAE counterpart). Layer choice: pick the SAE's layer, not your usual extraction layer — eliminates one confound.

Distinct from nov10-sae_decomposition (decomposes trait vectors *into* SAE features, doesn't validate against them) and dec16-sae_circuit_tracing (cross-validates layer-of-emergence, not direction). Connects to oct29-cka_method_agreement (method agreement framing) and dec17-validation (validation hierarchy).
...
## mar15-detection_layer_profiling
Find best *detection* layer per trait (vs best *steering* layer) using persona detection datasets. Prior results: text-register detection 91.7%, system prompt conditioning 24.2% (chance), LoRA weight changes 78%. Key finding: vectors detect text content and weight changes but not system prompt conditioning (attention attenuates by deep layers; LoRA changes affect every layer). 10-trait affective layer audit revealed three categories: naturally affective (joy, boredom — genuine emotion at early/mid layers), narration-first with affective sweet spot (anxiety, vulnerability — narrated early, genuine mid), and never affective (disgust, disappointment — narrated→tonal only). Internal-vs-surface extraction axis: internal lock-in (emotion/thought) → surface steering works; surface extraction → internal steering is weaker. Dual-vector extension: extract internal + surface vectors per trait, compare detection profiles by layer.
...
**Testbed: Haskins, Chughtai, Engels 2026 — "Training on Documents About Monitoring Leads To CoT Obfuscation"** ([LessWrong post](https://www.lesswrong.com/posts/training-on-documents-about-monitoring-leads-to-cot-obfuscation), [code](https://github.com/Reih02/cot-obfuscation-interim)). SDF on monitoring documents produces 31-47% CoT monitor evasion on gpt-oss-120b and Kimi K2, no RL needed. Models restructure CoT to look innocent while still being deceptive. Crucially: obfuscated CoTs still functionally encode deceptive intent (50-token prefix injection into base model → 88% deception rate). **This is the cleanest available testbed for unverbalized behaviors** — the model IS concealing, the CoT DOESN'T show it, activations SHOULD.

Repo ships per-sample JSONs with full CoTs + monitor classifications for every condition (behavioral, obfuscating, no-hints, no-reasoning, prefix injection, Kimi K2 replication). Key files: `phase_b_filtered.json` (107 consistently-undetected deceptive scenarios), `phase_c_prefix_results.json` (prefix injection results). Training data on HuggingFace under `Reih02/*`. LoRA weights not shipped (trained via Tinker API), but configs are in scripts — reproducible with standard PEFT on gpt-oss-120b.

**Concrete experiments:**
1. **Lowest effort — probes on shipped CoTs:** Run concealment/deception probes on the obfuscated CoTs from `phase_b_filtered.json` through base gpt-oss-120b. Do probes fire despite text looking innocent? This is the "probes detect what text monitors miss" result.
2. **Model diff:** Behavioral model vs obfuscating model is a contrastive pair (same base, different SDF). Diff activations on identical inputs to find a "monitor-awareness" or "obfuscation mode" direction. Our pipeline already does this.
3. **Prefix injection interp:** Their experiment shows 50 obfuscated tokens induce 88% deception in base model, but base model continuations become detectable again (47.9% detection). What happens in activation space at the handoff point? Does the concealment direction persist through the obfuscated prefix but spike when the base model takes over?

Sandbagging results are weaker (negligible undetected sandbagging improvement) and per-sample CoTs not shipped for that task. Focus on deception task.
