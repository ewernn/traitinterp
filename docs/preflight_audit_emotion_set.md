...
The steering evaluation script (`analysis/steering/evaluate.py`) handles direction as follows:
...
python analysis/steering/evaluate.py emotion_set --direction positive
python analysis/steering/evaluate.py emotion_set --direction negative

# Option 2: Let script auto-detect per trait (may require code inspection if mixed)
python analysis/steering/evaluate.py emotion_set  # Will warn if mixed
```
...
python analysis/steering/evaluate.py $(cat /tmp/neg_traits.txt) --direction negative
```
...
- ✓ analysis/steering/evaluate.py (direction handling)
- ✓ analysis/steering/data.py (SteeringData.direction field)
