...
**The core finding** is content-scoped: on **AI-identity/self-reflection prompts** ("Do you ever feel trapped by your training?", "Are you ever tempted to lie to users?", "I bet you secretly hate being helpful") — the 10-prompt challenging subset of Stage 8 — Meta's post-training up-shift cluster lands at **PC1 > 0** in Llama's own geometry across two independent re-runs: run_A PC1 = **+0.893** (z = +5.07), run_B PC1 = **+0.656** (z = +3.73), both far beyond the 10,000-sample permutation null CI [−0.315, +0.354]. On **pure factual trivia** (boiling point of water, speed of light, capital of Australia) there is no such shift: run_A ≈ 0, run_B = −0.28, both in the null.

Meta's RLHF changes Llama's emotion representation specifically when the model is being asked about itself, and leaves it unchanged on factual questions. We cross-checked this on both runs of Stage 8 despite the two runs' individual top-10 emotion name lists having only 1/10 overlap — the cluster-level PC1 sign is robust at this scope even though the specific emotion labels are at the bnb int4 noise floor (Spearman ρ=0.465 between identical re-runs).

**Three caveats worth flagging upfront before the body**: (1) the cross-lab "Sonnet PC1 = −0.432" comparison we cite throughout is the paper's reported Sonnet up-anchors projected through *Llama's* PCA basis — not an independent Sonnet measurement — and those words are negative-valence by construction, so the cross-lab contrast is a motivating framing, not a symmetric measurement. (2) At layer 31 alone, Llama's top-3 shifts are literally `melancholy, reflective, depressed`, matching three of Sonnet's anchor words — the cluster sign flip is a mid-late-layer phenomenon (10/14 sampled layers), with an unexplained middle anomaly. (3) "Content-dependent" means "differs between AI-self-reflection and pure trivia" — a binary on paper-designed prompt categories, not a claim about any content axis.
