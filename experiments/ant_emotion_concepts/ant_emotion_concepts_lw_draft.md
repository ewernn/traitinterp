...
1. **The "averaged-both" framing that earlier drafts used was misleading.** It mixed two subsets that behave differently. The robust claim is challenging-specific.
...
This parallels how humans' emotional tone varies with conversation content: the same person expresses different emotion profiles in work meetings vs in grief counseling. The content-dependent property is a plausibly-universal feature of post-training, not a Llama-specific accident.
...
**The load-bearing empirical content is "Llama's post-training up-cluster sits in the positive-valence half of Llama's own PC1 axis."** Four different scoring methods applied to Llama all give PC1 > 0, ranging from +0.134 to +0.856. This is a within-Llama measurement; the cross-run verification at run_A=+0.856 / run_B=+0.517 is documented in §"Post-training direction" above and lives in `pc1_stability_verification.json`.
...
**Stated as a within-Llama finding**: Meta's post-training moves Llama's emotion up-cluster into the positive-valence half of Llama's own PC1 axis (run_A PC1 = +0.856, run_B PC1 = +0.517, sign-stable despite 0/10 name-level overlap across two independent scripts). Stated as a framing: this is consistent with a different design choice than what the paper reports for Sonnet, where the up-anchors are negative-valence words, but the cross-lab comparison is asymmetric (we have a measurement on Llama, we have a word list for Sonnet) and the sign flip on Sonnet's side is nearly a lexical consequence of the words Anthropic chose to report rather than an independent geometric measurement. The down-direction (what Meta's RLHF suppresses) is asymmetrically weaker in our data. The cluster-level PC1 sign is the robust unit of comparison within Llama; specific emotion names within each cluster are one-run illustrative.
...
**The verified empirical content of this post is a within-Llama measurement**: Meta's post-training shifts Llama's emotion up-cluster toward positive valence in Llama's own PC1 basis, with run_A at PC1 = +0.856 and run_B at PC1 = +0.517 — both beyond a permutation null CI of [−0.315, +0.354] for a 10-of-171 random draw, sign-stable across two independent scripts, *despite zero overlap at the individual emotion-name level between the two runs*. The cluster-level within-Llama sign measurement is direct and has specific numbers behind it.
