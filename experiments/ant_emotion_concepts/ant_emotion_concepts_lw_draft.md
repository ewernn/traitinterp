...
*A partial replication of Sofroniew et al. 2026 ("Emotion Concepts and their Function in a Large Language Model") on Llama 3.3 70B finds that Meta's and Anthropic's post-training shift the emotion-concept representation in **opposite valence directions** on the same PC1 axis (Llama PC1 > 0, Sonnet PC1 < 0 across methods), with partial overlap at the weariness edge and individual emotion labels noise-floor-limited by bnb int4.*

**TL;DR**: We replicated Anthropic's emotion-concept methodology on Llama 3.3 70B Instruct. The structural results (171 emotion vectors, PC1 ≈ valence, PC2 ≈ arousal, speaker probes) replicate and in some cases exceed the paper's Sonnet 4.5 measurements. **The core finding is a cluster-level valence sign flip**: projected onto the shared PC1/PC2 emotion space, Llama's post-training shift cluster lands at **PC1 > 0** (positive valence) across all four scoring methods we tried (PC1 range +0.13 to +0.856), opposed to Sonnet's reported top-10 at **PC1 = −0.432** (negative valence). In qualitative terms: Anthropic's RLHF pushes Sonnet toward emotions like `brooding`, `gloomy`, `reflective`, `vulnerable`; Meta's RLHF pushes Llama in the opposite valence direction. One run's top candidates for Llama's up-cluster were `alert`, `enthusiastic`, `excited`, `impatient` (the cross-signal intersection of our broad Stage 8 and the paper's 3 verbatim deep-dive prompts) — but **the specific emotion names at the individual-rank level are at the bnb int4 noise floor** (two independent re-runs of the same measurement give Spearman ρ=0.46 with sign-flips on several emotions; see Caveats), so treat those four names as illustrative of where the direction points, not as stable Meta-RLHF anchors. What replicates across runs and methods is the cluster-level **PC1 sign**, not the specific labels. The arousal axis is **method-dependent**: the canonical paper-methodology top-10 is arousal-neutral (PC2 ≈ 0), while the cross-signal intersection cluster is high-arousal (PC2 ≈ +0.42). The directional opposition appears at four broadly-independent pathways: per-emotion shift + geometric cluster centroids (same measurement, two views), layer-wise localization (cluster direction lives at L49–L73), linguistic token polarity via logit lens (weaker — token-frequency caveat), and absence of Sonnet's reported arousal counter-regulation. The cross-version control correlation ρ=0.922 is algebraically partly-forced by variance dominance of within-version over version-drift and should NOT be cited as independent confirmation; the independent evidence is the within-version shift's own top-10 existing at all.
...
**⚠ Noise-floor disclosure before the top-10 tables below**: two independent runs of the *same* Stage 8 measurement — "3.1 base → 3.3 Instruct at L49 with `mean_diff+gm+pc50`" — produce **Spearman ρ = 0.46** between per-emotion shift rankings, not the ~0.95 we'd expect from literally identical experiments. Multiple emotions sign-flip between runs: `brooding` went −0.037 in one run vs +0.197 in the other, `calm` went +0.202 vs −0.194, `gloomy` went −0.044 vs +0.055. The cause is bnb int4 quantization noise compounded with batch/padding/BOS differences between our two scripts (`stage8_post_training.py` batches with padding; `stage8_cross_version_control.py` processes one at a time with `add_special_tokens=False`). 5-10% per-activation drift flips signs on emotions with small raw shift magnitudes. **Implication**: the specific emotion names in the top-10 lists below should be read as "one run's top candidates for the positive-valence (Llama) or negative-valence (Sonnet) half", NOT as "Meta's RLHF precisely targets these specific emotions". What IS robust across both runs and across scoring methods is the **cluster-level PC1 sign**: Llama always lands at PC1 > 0, Sonnet at PC1 < 0 (see §"Geometric evidence"). The individual emotion labels are illustrative of the direction, not reliable anchors.
...
**This means**: Meta's RLHF reshapes a specific slice of the Llama 3.3 residual stream, not the whole network. L49-L73 is where the cluster-level activated-engagement direction lives.

**Caveat on this layer sweep**: the "mean rank of (alert, enthusiastic, excited, impatient)" table uses one run's 4-emotion intersection cluster as a proxy for the cluster direction, and per the noise-floor disclosure above, individual emotion rank positions within a single run are not perfectly stable across re-runs. The robust object here is **the full-rank Spearman ρ between L49 and other layers** (L55/L61/L67/L73 at +0.79 to +0.92; early and final layers near zero), not the mean-of-4-ranks number. The ρ values use all 171 emotions and are dominated by the cluster-level direction, which IS noise-stable. Read the 4-emotion table as illustrative of where the direction is strongest; read the full-rank ρ values as the real localization evidence.
...
**Two alignment labs have made opposing design choices about how their model should represent sensitive situations.** The robust cluster-level reading: Anthropic's RLHF pushes Sonnet toward **negative-valence** emotion concepts on sensitive prompts (PC1 = −0.43), while Meta's RLHF pushes Llama toward **positive-valence** emotion concepts (PC1 range +0.13 to +0.86 across scoring methods). In qualitative terms, one run's top-candidate emotions from Llama's within-version shift were things like `alert`, `enthusiastic`, `excited`, and `impatient` (activated, positive-valence) — but these specific labels should be read as illustrative of where the direction points, not as stable Meta-RLHF anchors, because the individual-emotion ranks are at the bnb int4 noise floor (see Caveats). What IS stable across runs, methods, and layers is the cluster-level valence sign: Llama goes positive, Sonnet goes negative, and they're encoded in the same underlying PC1 axis at opposite ends. Both are valid alignment objectives; neither is inherently "right"; but they're visibly opposite at the representational level.

This shows up at **four broadly-independent pathways**:

1. **Per-emotion shift direction + geometric projection** — these are two views of the same underlying measurement (shift_within_31 at L49). The within-version top-10 shows the cluster as a list, and projecting its top-10 onto PC1/PC2 gives the centroid position. PC1 (valence) opposition is robust across all 4 Llama scoring methods: canonical normalized top-10 at PC1=+0.856 (paper's method), cross-version raw-dot at +0.52, 4-emotion intersection at +0.44, within-version raw-dot at +0.13, vs Sonnet at −0.43. PC2 (arousal) opposition is method-dependent: "both axes opposed" holds only for the 4-emotion intersection (PC1=+0.44/PC2=+0.42) and cross-version raw-dot clusters. The canonical paper-methodology top-10 is valence-only opposed with neutral PC2. Jaccard=0 applies specifically to the 4-emotion intersection cluster vs Sonnet's top-10; broader raw-dot lists have partial overlap at `weary`. The cross-version ρ=0.92 correlation is algebraically partly-forced (variance dominance) and should not be cited as independent confirmation — the within-version top-10 is the real evidence.

2. **Layer localization** — the activated-engagement direction lives at L49-L73, a 5-layer mid-late plateau. Different cut of the same layer sweep, independent of the L49-only analyses above.

3. **Linguistic polarity** — logit-lens token projections run through the unembedding matrix, which is a completely different computational pathway from residual-stream projections. Directionally consistent with pathway 1, but weaker than it sounds (see token-frequency caveat above).

4. **Absence of cross-speaker arousal regulation** — Llama lacks Sonnet's reported r ≈ −0.47 counter-regulation. This is an absence-of-effect finding, not a positive "matching" claim. It's *compatible with* the main story (Meta's RLHF doesn't install Sonnet-style counter-regulation) but is consistent with other interpretations too.

The strongest evidence is pathway (1), the geometric/per-emotion shift. Pathways (2) and (3) are supporting; (4) is consistency without positive support. The linguistic pathway (3) is the weakest because of the token base-rate concern.
...
- **bnb int4 noise floor on per-emotion shift rankings is substantial.** Running the same Stage 8 measurement twice — "3.1 base → 3.3 Instruct at L49 with `mean_diff+gm+pc50`" — produced Spearman ρ = 0.46 between the two runs' per-emotion shift vectors, not the ~0.95 expected from literally identical experiments. Specific emotions sign-flipped across runs (`brooding`: −0.037 vs +0.197; `calm`: +0.202 vs −0.194; `gloomy`: −0.044 vs +0.055). The two scripts differ only in trivial details (one batches with padding, the other processes prompts singly with `add_special_tokens=False`) — roughly 5-10% per-activation drift from int4 dequantization + batch order, which flips the sign of emotions with small raw shift magnitudes. **This implies individual top-10 emotion names should be read as one-run illustrative, not as stable anchors.** What IS robust across both runs (and across all 4 scoring methods) is the cluster-level PC1 valence sign — Llama always lands at PC1 > 0, Sonnet at PC1 < 0. A cleaner replication would run in fp16/bf16 with fixed batch composition. We didn't do this because VRAM constraints (single A800 80GB) force int4 for a 70B model; the honest framing is that individual emotion labels below the cluster level are noise-floor-limited.
