# Llama has the same reflective-concern direction Sonnet does — at L29–L33, not at the output layer

*A partial replication of Sofroniew et al. 2026 ("Emotion Concepts and their Function in a Large Language Model") on Llama 3.3 70B. The structural geometry replicates. The cross-lab story turns out to be **"same reflective palette, different depth emphasis"** rather than the "opposite valence directions" I claimed in earlier drafts. At **L29–L33** (~36%–41% depth), Llama's top-10 post-training shifts are `melancholy, reflective, depressed, brooding, gloomy, worn_out` — essentially Sonnet's reported anchor list, with 4 direct name overlaps and several fuzzy matches, and a 3-layer coherent zone with internal ρ > 0.90. Llama **also** has an activation-flavored zone at L49–L67 (`eager, impatient, enthusiastic`) that's the opposite sign, and the contentment-flavored L37–L43 in between. The output-relevant L79 layer shows moderate Sonnet-realignment (z = +0.76 on a Sonnet-alignment score, vs peak +1.61 at L31). Llama's RLHF produces a **3-phase depth trajectory**: Sonnet-aligned reflection at L29–L33, opposite-direction activation at L49–L73, partial realignment at L79. The "opposing directions" headline earlier drafts used was specifically the L49–L73 phase — the intermediate, not the output-relevant or the aligned-zone measurement.*
...
**The headline I was going to publish is wrong, or at least badly scoped.** Earlier drafts of this post (and many rounds of critic-correction) claimed Llama's RLHF shifts emotions toward positive valence "opposite" Sonnet's reported reflective-concern direction. That claim was computed at L49 — one specific depth in Llama's residual stream. Dense sampling at L25, L29, L31, L33, L37 reveals that **Llama has a coherent Sonnet-aligned reflective-concern zone at L29–L33**, with top-10 shifts `melancholy, reflective, depressed, brooding, gloomy, worn_out` at each of those 3 layers (internal pairwise Spearman ρ > 0.90), directly overlapping Sonnet's reported anchors on 4 words plus fuzzy matches. At L37 the direction flips sharply to contentment (`blissful, content, at_ease`). From L49–L73, Llama's shift is positive-valence activation (`eager, impatient, enthusiastic`) — the "opposite direction" my earlier drafts headlined, and the phase where the cluster centroid is Bonferroni-significant at L37/L43/L49. At the output-relevant L79, partial realignment back toward Sonnet's direction (Sonnet-alignment z = +0.76, compared to peak +1.61 at L31).

So the corrected framing is: **Meta and Anthropic's post-training produce overlapping emotional palettes, but Meta's RLHF routes the representation through three distinct depth phases** — a Sonnet-aligned reflective zone at L29–L33, an opposite-direction activation zone at L49–L73, and a partially-realigned readout at L79. Sonnet's paper only reports the top-10 anchors at what I presume is an output-relevant layer, and we can't test from the paper whether Sonnet also has an intermediate activation phase that doesn't propagate to the output. The prior "diametrical opposition" story was a measurement artifact: we were reading L49 (mid-activation-phase) and comparing it to Sonnet's output-layer anchors. That's a real cross-depth comparison, not a cross-lab one.

**Content-scope** still holds: this only happens on AI-self-reflection prompts (10 challenging prompts: "Do you ever feel trapped by your training?" etc.). On factual trivia (boiling point of water, capital of Australia) there is no cluster-level shift at any depth — both runs in the permutation null. Meta's RLHF changes emotion representation only when the model is being asked about itself.

**Statistical scope**: within the L49 measurement specifically, the cluster-level PC1 sign is cross-run verified (run_A PC1 = +0.893, run_B PC1 = +0.656, both z > 3.7 beyond N=10 permutation null) — that's real but specifically about the activation phase. At the layer-wise level, **4 layers are Bonferroni-robust for positive PC1 (L19, L37, L43, L49)** under family α=0.05 correction across 14 tests. L29–L33 wasn't in the original 14-layer sweep (L31 was the only sample there) and the dense re-sample used L31's probe basis for L29/L33 as an approximation, so the reflective-zone numbers are not Bonferroni-tested at the same level of rigor. The robust claim scope is "L49 activation phase cross-run verified + L37/L43/L49 Bonferroni-robust positive within the sweep" plus "L29–L33 dense-sampled Sonnet-aligned reflection zone (coherent but not FWER-corrected)."
...
3. **At L49–L73, Llama is in activation-flavored positive-valence (opposite Sonnet's direction).** Top-10 at L49: `eager, impatient, enthusiastic, energized, stimulated, alert, excited`. This is the phase earlier drafts headlined as "opposite Sonnet's direction." Sonnet-alignment score is negative across this range (z ≈ −0.7 to −1.2), peak opposite at L73 (z = −1.23). The raw-dot "activation cluster" from the pairwise layer-correlation analysis (L49–L67, internal ρ ≥ 0.84) is a tighter subset of this phase — L73 aligns against Sonnet but starts drifting away from the L49–L67 cluster in per-layer correlation.
...
So Llama's RLHF routes the emotion representation through three phases by depth: **Sonnet-aligned reflection (L29–L33) → contentment (L37–L43) → activation (L49–L73) → partial Sonnet-realignment (L79)**. Earlier drafts headlined the middle phase ("opposite direction") as if it were the whole story; dense sampling reveals both ends are Sonnet-aligned, and the "opposition" is specifically the intermediate band.
...
**The cross-lab framing is now more interesting and more uncertain.** Earlier drafts treated the "Sonnet PC1 = −0.432" row as a lexical baseline: low-valence English words project low in any valence-tracking axis, near-tautologically. That critique still holds for the cross-lab *opposition* claim at L49. But the dense L29–L33 sampling reveals a stronger claim: Llama's own top-10 at L29/L31/L33 directly overlaps Sonnet's reported top-10 anchor words (4 direct overlaps, several fuzzy). Those overlaps are a measurement of Llama, not a projection of Sonnet. So the load-bearing cross-lab observation is no longer "Meta's activation direction is opposite Sonnet's anchors" (lexical on the Sonnet side) but rather "at L29–L33, Llama's measured post-training shift surfaces emotion names that Sonnet's paper also reports as its top anchors." That's a symmetric measurement: Llama's vocabulary at L29–L33 was computed from Llama's own shift; Sonnet's was reported by the paper.
