...
1. **Within raw-dot scoring, Meta's post-training produces two near-orthogonal positive-valence clusters at adjacent depths.** Pairwise Spearman ρ of the 171-dim layer-wise shift vectors shows L37–L43 form a tight cluster (internal ρ=0.89, top emotions: blissful/content/at_ease/relaxed/satisfied — "contentment") and L49–L67 form another (internal ρ≥0.84, top emotions: eager/impatient/enthusiastic/energized/alert — "activation"), with cross-cluster ρ≈0.16–0.46. Both clusters are positive-valence; both differ from Sonnet's reported reflective-concern anchors. This is a genuine mechanistic finding about RLHF depth-structure.
...
Statistically, the positive-PC1 signature is significant vs each layer's own permutation null at **L43 (z=+5.36), L49 (z=+2.93), and L55 (z=+1.98)** — a 3-layer window, not the broader plateau earlier drafts claimed. At L61–L73 the cluster centroid is still positive but not distinguishable from chance.
...
**⚠ Two phenomena layered on top of each other** — separating them matters for interpretation:

**Phenomenon 1: within a single scoring convention, there are two adjacent-depth clusters.** The layer sweep (`results/stage8_layer_sweep.json`) uses raw-dot scoring at each of 14 sampled layers. Pairwise Spearman ρ of the 171-dim layer-wise shift vectors within raw-dot:
...
This is a real depth-phenomenon of raw-dot-scored Stage 8 shifts. Whether canonical normalized scoring produces the same two-cluster depth structure is untested — we'd need canonical scoring at L37, L43, L55, L61, L67 (we only have it at L49).

**Phenomenon 2: at a single layer (L49), two scoring conventions produce disjoint top-10s.** Our two Stage 8 runs — `stage8_post_training.py` (canonical normalized) and `stage8_cross_version.py` (raw-dot) — both measured at L49, produced top-10 emotion lists with **0/10 overlap** at the averaged-both level and 1/10 overlap on challenging-only (`thrilled` in both). The full 171-dim shift vectors correlate only ρ=0.465 across runs.
...
1. **Within raw-dot scoring**: two distinct positive-valence clusters exist at adjacent depths (L37–L43 and L49–L67). This is a genuine mechanistic finding.
...
**And — interestingly — that 3-layer significance window contains emotions from both raw-dot clusters.** L43 is inside the L37–L43 contentment cluster (raw-dot shift there peaks at PC1 = +0.947 with top emotions `satisfied, cheerful, jubilant`); L55 is inside the L49–L67 activation cluster (raw-dot top at L55 is `impatient, stimulated, eager`); L49 is a raw-dot shift vector whose nearest neighbors in the layer sweep are L55 (ρ=0.92) and L61 (ρ=0.86), so within raw-dot L49 is essentially the upper edge of the activation cluster, not a "midpoint." **Meta's post-training isn't producing one direction along a plateau — within raw-dot scoring it's producing two adjacent-depth positive-valence signatures, both of which happen to have layers in the statistically-significant L43/L49/L55 window.** (The 3-layer significance window is not, itself, a boundary state between the two clusters — it's just the set of layers where the top-10 centroid clears the permutation null; L43 clears it via the contentment cluster, L49 and L55 clear it via the activation cluster.)
...
1. **On AI-self-reflection prompts, Meta's post-training shifts Llama's emotion cluster to PC1 > 0** in Llama's own geometry, significant at L43/L49/L55 (3-layer window). Run_A PC1 = +0.893 (z=+5.07), run_B PC1 = +0.656 (z=+3.73) on challenging-only. Both beyond the permutation null CI. On factual trivia there is no shift.

2. **Within raw-dot scoring across the layer sweep, Meta's post-training produces two near-orthogonal positive-valence clusters at adjacent depths** — "contentment" (blissful/content/at_ease) at L37–L43 (internal ρ=0.89) and "activation" (eager/impatient/enthusiastic) at L49–L67 (internal ρ≥0.84), with cross-cluster ρ≈0.16–0.46. This is a depth-phenomenon within a single scoring convention. Whether canonical normalized scoring produces the same two-cluster depth structure is untested.
