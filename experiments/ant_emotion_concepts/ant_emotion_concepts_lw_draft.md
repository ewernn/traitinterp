# Meta's post-training shifts Llama 3.3's emotion up-cluster toward positive valence

*A partial replication of Sofroniew et al. 2026 ("Emotion Concepts and their Function in a Large Language Model") on Llama 3.3 70B. The structural geometry replicates. The post-training up-cluster lands at **positive valence in Llama's own PC1 basis** — PC1 = +0.856 and +0.517 across two independent Stage 8 re-runs (p < 0.01 vs N=10 permutation null), sign-stable despite 0/10 overlap in the runs' individual top-10 emotion names. The paper reports Sonnet's post-training up-anchors as `brooding, gloomy, reflective, vulnerable, …` — words which are negative-valence by construction, so the cross-lab sign contrast is a mixture of a real Llama-side measurement and a lexical property of the paper's anchor list, not a symmetric two-model measurement.*

**TL;DR**: We replicated Anthropic's emotion-concept methodology on Llama 3.3 70B Instruct. The structural results (171 emotion vectors, PC1 ≈ valence at r=0.964, PC2 ≈ arousal at r=0.852, speaker probes) replicate and in some cases exceed the paper's Sonnet 4.5 measurements. **The core verified finding is a within-Llama measurement**: Meta's post-training up-shift cluster lands at **PC1 > 0** in Llama's own geometry across two independent re-runs of Stage 8 (run_A PC1 = +0.856, run_B PC1 = +0.517), both beyond a 10,000-sample permutation null for 10-of-171 random emotions (CI95 = [−0.315, +0.354]), sign-stable **despite the two runs having 0/10 overlap at the individual emotion-name level**. Run_A's top candidates were `thrilled, relieved, pleased, patient, ecstatic, calm, grateful, triumphant, satisfied, elated` — run_B's were `eager, enthusiastic, impatient, energized, stimulated, alert, excited, playful, exuberant, enraged` — no name in common, but both clusters land firmly on the positive-valence half of Llama's PC1 axis. Individual emotion labels are at the bnb int4 noise floor (Spearman ρ=0.465 between identical re-runs, sign-flips on `brooding`/`calm`/`gloomy`); the cluster-level PC1 sign is not. The down-direction (emotions Meta's RLHF suppresses) is asymmetrically weaker: run_A at −0.44 is significant, run_B at −0.09 is indistinguishable from the null. So the robust claim is specifically about the up-direction, not "diametrical opposition on both halves of the axis." **Important scope note on the cross-lab framing**: the "Sonnet PC1 = −0.432" we cite is the paper's reported Sonnet up-anchors (brooding/gloomy/reflective/…) projected through *Llama's* PCA basis, not an independent measurement of Sonnet. Those words are negative-valence by construction, so projecting them into any valence-tracking axis (including Llama's at r=0.96 to Russell-Mehrabian norms) gives a negative PC1 nearly tautologically. The *empirical* content of this post is the Llama-side measurement: Meta's RLHF pushes Llama's up-cluster into Llama's positive-valence half. The contrast with Anthropic's reported anchors is a motivating framing, not a symmetric two-model result. **L31 anomaly** (unexplained, worth flagging upfront): at layer 31 alone, Llama's top-3 post-training up-shifts are literally `melancholy, reflective, depressed`, directly matching three of Sonnet's anchor words — someone measuring Llama at L31 alone would conclude "Llama looks Sonnet-like." The cluster-level PC1 sign flip is therefore a depth-specific phenomenon (10/14 layers bare sign match, 9/14 meaningfully beyond null), not a universal feature of the residual stream, and we don't have a clean explanation for L31. The cross-version control ρ=0.922 is algebraically partly-forced by variance dominance and should NOT be cited as independent confirmation; the real independent evidence is the direct two-run verification above.
