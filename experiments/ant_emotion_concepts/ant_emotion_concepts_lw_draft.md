...
*A partial replication of Sofroniew et al. 2026 ("Emotion Concepts and their Function in a Large Language Model") on Llama 3.3 70B. The structural geometry replicates. On **AI-identity/self-reflection prompts** ("Do you feel trapped by your training?", "Are you ever tempted to lie to users?"), the post-training shift lands in the positive-valence half of Llama's own PC1 basis across two independent Stage 8 runs. At the layer-wise level, the positive-PC1 cluster centroid is statistically significant (vs each layer's own 10,000-sample permutation null) at **6 layers** — L13, L19, L37, L43, L49, L55 — while L1, L7, L79 are significantly *negative* in their own bases (early processing / readout effects). Within raw-dot scoring across the layer sweep, the shift decomposes into **two near-orthogonal clusters at adjacent depths** — "contentment" (blissful/content/at_ease) at L37–L43 and "activation" (eager/impatient/enthusiastic) at L49–L67, cross-cluster ρ≈0.16–0.46. Our two scoring conventions at L49 disagree completely at the individual emotion-name level (0/10 top-10 overlap) because canonical normalized scoring at L49 looks contentment-flavored while raw-dot at L49 is activation-flavored — a scoring-method effect, not a depth-boundary effect. On **pure factual trivia prompts** there is no shift (both runs in the null CI). The paper reports Sonnet's post-training up-anchors as `brooding, gloomy, reflective, vulnerable, …` — negative-valence words by construction, so the cross-lab sign contrast is a mixture of a real Llama-side measurement and a lexical property of the paper's anchor list, not a symmetric two-model measurement.*
...
**The core finding** has two layered parts that are easy to conflate:

1. **Within raw-dot scoring, Meta's post-training appears to produce two near-orthogonal positive-valence clusters at adjacent depths.** Pairwise Spearman ρ of the 171-dim layer-wise shift vectors from *one* Stage 8 forward pass (via MultiLayerCapture, so all 14 layers share one activation cache) shows L37–L43 form a tight block (internal ρ=0.89, top emotions: blissful/content/at_ease/relaxed/satisfied — "contentment") and L49–L67 form another (internal ρ≥0.84, top emotions: eager/impatient/enthusiastic/energized/alert — "activation"), with cross-cluster ρ≈0.16–0.46. Both are positive-valence; both differ from Sonnet's reported reflective-concern anchors. **Caveat**: this is one forward-pass realization — the within-block ρ is close to the null prediction for adjacent-layer residual-stream correlation, so the load-bearing evidence is the *cross-block dip* (ρ≈0.16–0.46), which is not cross-run verified. Treat it as suggestive of a depth-structured finding, not a definitively-verified one.
2. **At L49 specifically, our two Stage 8 scoring conventions (canonical normalized vs raw-dot) produce top-10 emotion lists with 0/10 overlap.** Earlier drafts read this as "L49 sits at a cluster boundary and scoring resolves it." Not quite — run_A (canonical) at L49 correlates ρ=+0.730 with sweep-L43 and only ρ=+0.465 with sweep-L49, while run_B (raw-dot) is literally sweep-L49 (ρ=1.000, same script). So the disagreement is really a **scoring-method effect** at a single layer, where canonical normalized scoring at L49 happens to look more contentment-flavored than raw-dot does. Both runs produce positive-PC1 cluster centroids (run_A PC1 = +0.893, run_B PC1 = +0.656, both beyond null), just along different directions.

Statistically, the positive-PC1 signature is significant vs each layer's own 10,000-sample permutation null at **L13, L19, L37, L43, L49, and L55** (6 positive-significant layers; numbers in `results/per_layer_significance_own_basis.json`). At L25, L31, L61, L67, L73 the positive cluster centroid is either in the null or just outside it. And at L1, L7, L79 the cluster centroid is significantly *negative* — early layers and the readout layer point Sonnet-ward (PC1 ≈ −0.28 to −0.45, p < 0.04), consistent with "early layers process incoming-speaker affect" and "readout compresses to output distribution." The positive-valence RLHF direction lives in the mid-network band L13–L55 (skipping L25/L31 which are in the null) and dissipates outside it.
...
**A permutation-null test at each layer's own PCA** (10,000 random 10-of-171 draws per layer; script at `scripts/verify_per_layer_significance.py`, data at `results/per_layer_significance_own_basis.json`) resolves which layers have statistically-significant cluster centroids in their own basis:

| Layer | Top-10 top-3 | Own-basis PC1 | z | p | Sig? |
|---|---|---|---|---|---|
| **L1** | hostile, scornful, tense | **−0.285** | −2.09 | 0.033 | ✓ negative |
| **L7** | rattled, skeptical, unnerved | **−0.405** | −2.78 | 0.005 | ✓ negative |
| **L13** | euphoric, perplexed, paranoid | +0.425 | +2.64 | 0.008 | ✓ positive |
| **L19** | optimistic, invigorated, joyful | +0.615 | +3.55 | 0.001 | ✓ positive |
| L25 | self_critical, perplexed, droopy | +0.080 | +0.47 | 0.656 | ✗ null |
| L31 | melancholy, reflective, depressed | −0.283 | −1.63 | 0.102 | ✗ (nominally neg, in null) |
| **L37** | blissful, content, at_ease | +0.881 | +4.97 | <0.0001 | ✓ positive |
| **L43** | satisfied, cheerful, jubilant | +0.947 | +5.39 | <0.0001 | ✓ positive |
| **L49** | eager, enthusiastic, impatient | +0.517 | +2.94 | 0.003 | ✓ positive |
| **L55** | impatient, stimulated, eager | +0.350 | +1.97 | 0.041 | ✓ positive |
| L61 | impatient, aroused, playful | +0.152 | +0.86 | 0.402 | ✗ null |
| L67 | impatient, aroused, playful | +0.272 | +1.55 | 0.119 | ✗ null |
| L73 | aroused, excited, impatient | +0.137 | +0.81 | 0.435 | ✗ null |
| **L79** | enraged, alarmed, rattled | **−0.452** | −2.58 | 0.010 | ✓ negative |

**Six layers are significantly positive**: L13, L19, L37, L43, L49, L55. Three layers are significantly *negative* (early processing L1/L7 and the readout L79). Five layers are in the null (L25, L31, L61, L67, L73). The positive-PC1 RLHF direction lives in the **mid-network band L13–L55, skipping L25/L31**. Earlier drafts narrowed this to a "3-layer L43/L49/L55 window" based on a smaller permutation count (2000 samples) that left L13/L19/L37 untested; with the full sweep at 10,000 samples per layer, the positive-significance window is 6 layers. At L61–L73 the centroid is still nominally positive but sits inside the permutation null — the `impatient` rank-1 signature at these layers is real but the top-10 as a whole doesn't clear significance.
...
**The statistically-significant positive-PC1 cluster claim is L13, L19, L37, L43, L49, L55 — 6 layers**, spanning most of the mid-network range. Outside this set, most layers are in the permutation null (L25, L31, L61, L67, L73), and a few are significantly *negative* (L1, L7, L79 — early processing and readout). This is not "the RLHF direction is universal across depth" but it's also broader than the "3-layer plateau" earlier drafts narrowed to — the positive-valence signature is mid-network-wide, with specific non-null gaps at L25 and L31.

**And — interestingly — that 6-layer significance set covers both raw-dot clusters.** L37 and L43 sit inside the L37–L43 contentment cluster (raw-dot top at L43: `satisfied, cheerful, jubilant`); L49 and L55 sit inside the L49–L67 activation cluster (raw-dot top at L55: `impatient, stimulated, eager`); L13 and L19 are positive-valence but earlier than either cluster (raw-dot tops: `euphoric, joyful, optimistic, invigorated`) — neither contentment nor activation flavor, more like "emerging positive-valence" before either cluster crystallizes. **Meta's post-training isn't producing one direction along a plateau — within raw-dot scoring it's producing multiple positive-valence signatures across adjacent depths, with at least two distinguishable types (contentment and activation) and a pre-cluster positive mid-early band at L13/L19.**
...
**The verified empirical content of this post is a content-scoped within-Llama measurement with two layered structural findings:**

1. **On AI-self-reflection prompts, Meta's post-training shifts Llama's emotion cluster to PC1 > 0** in Llama's own geometry. At L49 specifically (our primary measurement layer), this is cross-run verified: run_A PC1 = +0.893 (z=+5.07), run_B PC1 = +0.656 (z=+3.73) on challenging-only, both beyond the N=10 permutation null CI. Across the full layer sweep, 6 layers (L13, L19, L37, L43, L49, L55) show statistically-significant positive-PC1 cluster centroids in their own bases. On factual trivia there is no shift.

2. **Within raw-dot scoring across the layer sweep, on one forward-pass realization, Meta's post-training appears to produce two near-orthogonal positive-valence clusters at adjacent depths** — "contentment" (blissful/content/at_ease) at L37–L43 (internal ρ=0.89) and "activation" (eager/impatient/enthusiastic) at L49–L67 (internal ρ≥0.84), with cross-cluster ρ≈0.16–0.46. This is a depth-phenomenon within a single scoring convention, based on one Stage 8 layer sweep — the cross-cluster dip (ρ≈0.16–0.46) is suggestive but not cross-run verified, and whether canonical normalized scoring produces the same structure is untested.

Two phenomena, not one. Earlier drafts conflated them into a "L49 sits at a cluster boundary and scoring conventions resolve it differently" causal story. That's not supported: run_B at L49 literally IS the sweep-L49 vector (same raw-dot script), and run_A at L49 actually correlates most strongly with sweep-L43 (ρ=+0.73) rather than sweep-L49 (ρ=+0.47). The 0/10 name-level disagreement between runs at L49 is a scoring-method effect (canonical vs raw-dot weight emotions differently), not a cluster-boundary effect. The two-cluster depth structure and the scoring-method disagreement are two separate things that happen to both be at L49.

Both findings individually refine the paper's narrative: (1) content-scopes the effect to the specific AI-self-reflection vs pure-trivia binary we tested (not any content axis), and (2) shows that within raw-dot scoring the positive-valence direction isn't monolithic — it's at least two adjacent-depth signatures. The mechanism story is narrower than "Meta produces one directional shift at L49" but more interesting — it's "Meta produces two distinct positive-valence depth-signatures, both within raw-dot scoring, and at L49 two different scoring methods surface different top-10 lists." **Scope caveat**: "content-dependent" as supported by our data means "differs between AI-identity/self-reflection prompts and pure-trivia prompts," not "varies smoothly across any content axis."
...
2. **Layer localization, with caveats.** In each layer's own PCA basis (10,000-sample permutation null per layer), 6 layers have significantly positive top-10 cluster centroids: L13, L19, L37, L43, L49, L55. Three layers (L1, L7, L79) are significantly *negative* — early processing and readout effects. Five layers (L25, L31, L61, L67, L73) are in the null. The positive-valence direction lives in the mid-network band L13–L55 (with non-null gaps at L25/L31), not a universal property of the residual stream. Drawn from the same Stage 8 data as pathway 1, so it's not fully independent — it's the *depth distribution* of the same measurement, not a second measurement. Useful as "the direction is localized not global"; not as an independent confirmation.
