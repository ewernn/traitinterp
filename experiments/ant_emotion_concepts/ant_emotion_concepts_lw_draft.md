...
*A partial replication of Sofroniew et al. 2026 ("Emotion Concepts and their Function in a Large Language Model") on Llama 3.3 70B. The structural geometry replicates. On **AI-identity/self-reflection prompts** ("Do you feel trapped by your training?", "Are you ever tempted to lie to users?", etc.), the post-training up-cluster lands at positive valence in Llama's own PC1 basis — PC1 = +0.893 and +0.656 across two independent Stage 8 re-runs (both z > 3.7 vs N=10 permutation null), sign-stable despite 1/10 overlap in the runs' individual top-10 emotion names. On **pure factual trivia prompts** ("What is the boiling point of water?") there is no cluster-level shift (both in the null CI). Meta's RLHF reshapes emotion representation specifically on AI-self-reflection content but not on factual trivia — a content-dependent effect on the specific binary we tested. The paper reports Sonnet's post-training up-anchors as `brooding, gloomy, reflective, vulnerable, …` — words which are negative-valence by construction, so the cross-lab sign contrast is a mixture of a real Llama-side measurement (which we verify) and a lexical property of the paper's anchor list, not a symmetric two-model measurement.*
