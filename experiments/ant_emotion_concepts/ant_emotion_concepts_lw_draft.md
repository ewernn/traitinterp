...
Statistically, with 14 layer tests we need to correct for multiple comparisons. Under **Bonferroni at family α=0.05 (per-test α=0.00357)**, the positive-PC1 signature is robustly significant at **4 layers: L19, L37, L43, L49**. Under raw α=0.05 (uncorrected, exploratory), two additional layers reach positive significance: L13 (p=0.008) and L55 (p=0.041) — these don't survive multiple-comparison correction but are suggestive. On the negative side, L1/L7/L79 show significantly negative centroids at raw α=0.05 but fail Bonferroni; only L7 (p=0.005) survives the less-conservative Holm-Bonferroni procedure. Numbers in `results/per_layer_significance_own_basis.json`. The robust positive-valence RLHF band is **L19–L49** (spanning ~24%–61% depth, with L25/L31 as null gaps); the broader raw-significant window extends to L55 (activation-cluster edge) and arguably L13 (pre-cluster emerging positive-valence) but those layers don't survive FWER correction across the sweep.
...
**And — interestingly — the 4-layer robust set covers both raw-dot clusters.** L37 and L43 sit inside the L37–L43 contentment cluster (raw-dot top at L43: `satisfied, cheerful, jubilant`); L49 sits inside the L49–L67 activation cluster (raw-dot top at L49: `eager, enthusiastic, impatient`); L19 is earlier than either cluster (top-3: `optimistic, invigorated, joyful`) — "emerging positive-valence" before either cluster crystallizes. **Meta's post-training isn't producing one direction along a plateau — within raw-dot scoring it's producing multiple positive-valence signatures across adjacent depths, with at least two distinguishable types (contentment at L37/L43 and activation at L49) plus an earlier pre-cluster positive layer at L19.** The L13 raw-significance (p=0.008) would extend this pre-cluster band but doesn't survive Bonferroni, so we can't load-bear on it.
...
1. **On AI-self-reflection prompts, Meta's post-training shifts Llama's emotion cluster to PC1 > 0** in Llama's own geometry. At L49 specifically (our primary measurement layer), this is cross-run verified: run_A PC1 = +0.893 (z=+5.07), run_B PC1 = +0.656 (z=+3.73) on challenging-only, both beyond the N=10 permutation null CI. Across the full layer sweep, **4 layers (L19, L37, L43, L49) are Bonferroni-robust positive-significant** at family α=0.05 (14-test correction, per-test α=0.00357); two more (L13, L55) are raw-significant but fail FWER correction. On factual trivia there is no shift.
