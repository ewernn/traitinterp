# Meta's post-training shifts Llama 3.3's emotion up-cluster toward positive valence — specifically on sensitive prompts

*A partial replication of Sofroniew et al. 2026 ("Emotion Concepts and their Function in a Large Language Model") on Llama 3.3 70B. The structural geometry replicates. On **challenging/sensitive prompts** the post-training up-cluster lands at positive valence in Llama's own PC1 basis — PC1 = +0.893 and +0.656 across two independent Stage 8 re-runs (both z > 3.7 vs N=10 permutation null), sign-stable despite 1/10 overlap in the runs' individual top-10 emotion names. On **neutral prompts** there is no cluster-level shift (run_A PC1 ≈ 0, run_B PC1 = −0.28, both in the null CI). Meta's RLHF reshapes emotion representation in a content-dependent way: it acts on sensitive prompts, not on everything. The paper reports Sonnet's post-training up-anchors as `brooding, gloomy, reflective, vulnerable, …` — words which are negative-valence by construction, so the cross-lab sign contrast is a mixture of a real Llama-side measurement (which we verify) and a lexical property of the paper's anchor list, not a symmetric two-model measurement.*

**TL;DR**: We replicated Anthropic's emotion-concept methodology on Llama 3.3 70B Instruct. The structural results (171 emotion vectors, PC1 ≈ valence at r=0.964, PC2 ≈ arousal at r=0.852, speaker probes) replicate and in some cases exceed the paper's Sonnet 4.5 measurements. **The core verified finding is a content-dependent within-Llama measurement**: on **challenging/sensitive prompts** (the paper's design target), Meta's post-training up-shift cluster lands at PC1 > 0 in Llama's own geometry across two independent Stage 8 re-runs — run_A PC1 = **+0.893** (z = +5.07, p < 0.0001), run_B PC1 = **+0.656** (z = +3.73, p = 0.0003) — both far beyond a 10,000-sample permutation null CI = [−0.315, +0.354]. On **neutral prompts** there is no such shift: run_A PC1 = −0.0002 (literally zero), run_B PC1 = −0.28 (in the null). Meta's RLHF changes emotion representation in a **content-dependent** way, acting on sensitive content but leaving neutral content's emotion axis essentially unchanged. The sign-stability claim is also robust at the individual emotion-name level in a weak sense: run_A's challenging top-10 is `thrilled, pleased, triumphant, relieved, proud, delighted, joyful, grateful, ecstatic, calm` and run_B's is `eager, enthusiastic, energized, excited, exuberant, stimulated, thrilled, impatient, alert, vibrant` — only 1/10 name overlap (`thrilled`), but both clusters project firmly to the positive-valence half of Llama's PC1 axis. Individual emotion labels are at the bnb int4 noise floor (Spearman ρ=0.465 between identical re-runs, sign-flips on `brooding`/`calm`/`gloomy`); the cluster-level PC1 sign on the challenging subset is not. The down-direction (emotions Meta's RLHF suppresses) is asymmetrically weaker: run_A at −0.43 is marginally significant, run_B at −0.28 is in the null. So the robust claim is specifically about what the up-cluster *amplifies* on challenging prompts, not a full "diametrical opposition" story. **Important scope note on the cross-lab framing**: the "Sonnet PC1 = −0.432" we cite throughout is the paper's reported Sonnet up-anchors (brooding/gloomy/reflective/…) projected through *Llama's* PCA basis, not an independent measurement of Sonnet. Those words are negative-valence by construction, so projecting them into any valence-tracking axis (including Llama's at r=0.96 to Russell-Mehrabian norms) gives a negative PC1 nearly tautologically. The *empirical* content of this post is the within-Llama measurement; the contrast with Anthropic's reported anchors is a motivating framing, not a symmetric two-model result. **L31 anomaly** (unexplained, worth flagging upfront): at layer 31 alone, Llama's top-3 post-training up-shifts are literally `melancholy, reflective, depressed`, directly matching three of Sonnet's anchor words — someone measuring Llama at L31 alone would conclude "Llama looks Sonnet-like." The cluster-level PC1 sign flip is therefore a depth-specific phenomenon (10/14 layers bare sign match, 9/14 meaningfully beyond null), not a universal feature of the residual stream, and we don't have a clean explanation for L31. The cross-version control ρ=0.922 is algebraically partly-forced by variance dominance and should NOT be cited as independent confirmation; the real independent evidence is the direct two-run challenging-subset verification above.
...
**But — and this is the load-bearing result — the cluster-level PC1 sign is robust across runs on the challenging-prompt subset.** (See §Content-dependence below for why we scope this to challenging rather than averaged. Short version: the averaged-both cluster PC1 also lands beyond null at +0.856 / +0.517, but that's a mixture of a strongly positive challenging-subset effect and a literally-zero neutral-subset effect, so the averaged numbers understate the cleanness of the challenging-only result.) On challenging prompts only, we computed the PC1 centroid of each run's top-10 against a 10,000-permutation null for N=10 random emotions from 171 (null CI95 = [−0.315, +0.354]):

- **run_A challenging up-cluster PC1 = +0.8934** (z = +5.07 vs null, p < 0.0001) — top-10: `thrilled, pleased, triumphant, relieved, proud, delighted, joyful, grateful, ecstatic, calm`
- **run_B challenging up-cluster PC1 = +0.6559** (z = +3.73 vs null, p = 0.0003) — top-10: `eager, enthusiastic, energized, excited, exuberant, stimulated, thrilled, impatient, alert, vibrant`

Both runs land at PC1 > 0 by multiple standard deviations on the challenging-only subset, with 1/10 name overlap (`thrilled`). The "cluster PC1 sign is robust" claim isn't a rhetorical save — it's a real empirical property of the measurement, confirmed by the cross-run verification in `results/pc1_stability_verification.json` (averaged-both) and the cross-scenario verification in `results/pc1_cross_scenario_verification.json` (challenging-only, which is the load-bearing result).
...
**The verified empirical content of this post is a content-scoped within-Llama measurement**: on challenging/sensitive prompts specifically, Meta's post-training shifts Llama's emotion up-cluster toward positive valence in Llama's own PC1 basis, with run_A at PC1 = +0.893 (z = +5.07) and run_B at PC1 = +0.656 (z = +3.73) — both far beyond a permutation null CI of [−0.315, +0.354] for a 10-of-171 random draw, sign-stable across two independent scripts despite only 1/10 name-level overlap. On neutral prompts there is no such shift (run_A ≈ 0, run_B = −0.28, both in the null). The content-dependence is itself a finding: Meta's RLHF reshapes emotion representation selectively, acting on sensitive content and leaving the neutral-prompt emotion axis essentially unchanged. This is both a mechanism claim (when does RLHF act?) and a direction claim (which way does it push?), and it's stronger than either alone — the paper's framing of RLHF as producing "emotional nuance" refines to "content-dependent emotional nuance, selectively deployed on the content classes that trigger sensitive responses."
