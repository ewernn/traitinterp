...
Input:
    ...
    (Generated by inference/project_activations_onto_traits.py)

Output:
    experiments/{experiment}/model_diff/{variant_a}_vs_{variant_b}/per_token_diff/{trait}/L{layer}/{prompt_set}/
...
def process_prompt(
    ...
    scores_a = proj_a['response']
    scores_b = proj_b['response']
...
def main():
    ...
    for trait in traits:
        ...
        # Output directory — include L{layer}/ when layer is known
        diff_base = exp_dir / 'model_diff' / f'{args.variant_a}_vs_{args.variant_b}' / 'per_token_diff' / trait
        if layer is not None:
            out_dir = diff_base / f'L{layer}' / args.prompt_set
        else:
            out_dir = diff_base / args.prompt_set
        ...
        for pf in prompt_files:
            ...
            result = process_prompt(pid, proj_a_file, pf, resp_file, trait, layer=layer)
