"""
Pre-extraction vetting: LLM-as-judge validates generated responses before extraction.
...
Usage:
    from utils.preextraction_vetting import vet_responses
    pass_rate = vet_responses(experiment, trait, model_variant, tokenizer=tok)
"""
...
def vet_responses(
    ...
    Args:
        estimate_trait_tokens: Estimate trait token positions (for adaptive position).
        ...
        tokenizer: HuggingFace tokenizer (required). Used for position-accurate scoring
            that matches extraction's token boundaries.
